...
// --- FlatMap variants ---
// These run the same acceptance tests but with EnableFlat() called first.
// The FlatMap implementation must produce identical results.

func TestAcceptanceFlatGetValueScalar(t *testing.T) {
	e := &Event{Fields: mapstr.M{"message": "hello"}}
	e.EnableFlat()
	v, err := e.GetValue("message")
	require.NoError(t, err)
...
func TestAcceptanceFlatGetValueNested(t *testing.T) {
	e := &Event{Fields: mapstr.M{
		"cloud": mapstr.M{"provider": "aws", "region": "us-east-1"},
	}}
	e.EnableFlat()
...
func TestAcceptanceFlatGetValueParentReturnsMap(t *testing.T) {
	e := &Event{Fields: mapstr.M{
		"cloud": mapstr.M{"provider": "aws", "region": "us-east-1"},
	}}
	e.EnableFlat()
...
func TestAcceptanceFlatPutValueAndGet(t *testing.T) {
	e := &Event{Fields: mapstr.M{}}
	e.EnableFlat()
	_, err := e.PutValue("cloud.provider", "aws")
	require.NoError(t, err)
...
func TestAcceptanceFlatDelete(t *testing.T) {
	e := &Event{Fields: mapstr.M{
		"cloud": mapstr.M{"provider": "aws", "region": "us-east-1"},
	}}
	e.EnableFlat()
...
func TestAcceptanceFlatDeepUpdate(t *testing.T) {
	e := &Event{Fields: mapstr.M{
		"agent": mapstr.M{"type": "filebeat"},
	}}
	e.EnableFlat()
...
func TestAcceptanceFlatDeepUpdateNoOverwrite(t *testing.T) {
	e := &Event{Fields: mapstr.M{
		"agent": mapstr.M{"type": "filebeat"},
	}}
	e.EnableFlat()
...
func TestAcceptanceFlatRender(t *testing.T) {
	e := &Event{Fields: mapstr.M{
		"message": "hello",
	}}
	e.EnableFlat()
	...
	// Fields should now be a nested mapstr.M
	v, _ := e.Fields.GetValue("cloud.provider")
	assert.Equal(t, "aws", v)
	v, _ = e.Fields.GetValue("agent.id")
	assert.Equal(t, "123", v)
	assert.Equal(t, "hello", e.Fields["message"])
}

func TestAcceptanceFlatCloneIndependence(t *testing.T) {
	e := &Event{
		Timestamp: time.Now(),
		Meta:      mapstr.M{"pipeline": "test"},
		Fields: mapstr.M{
...
func TestAcceptanceFlatProcessorChain(t *testing.T) {
	e := &Event{
		Timestamp: time.Now(),
		Fields: mapstr.M{
			"message": "test log",
	...
	// Render and verify nested structure
	e.Render()
	v, _ = e.Fields.GetValue("cloud")
	assert.Nil(t, v) // cloud was never set

	v, _ = e.Fields.GetValue("elastic_agent.id")
	assert.Equal(t, "agent-123", v)
}
