...
//
// cowMap values are stored in the event's SmallMap at top-level keys.
// Event accessor methods handle cowMap transparently.
type cowMap struct {
	shared mapstr.M
}

// newCowMap creates a cowMap wrapping shared data (internal use).
...
// Use this in processors to store shared metadata without per-event cloning.
func NewCowMap(shared mapstr.M) *cowMap {
	return &cowMap{shared: shared}
}
